...
const PrivacyPolicyPage = ({ language, onBack }: PrivacyPolicyPageProps) => {
  const t = translations[language]?.terms?.privacyPolicy;

  if (!t || !t.fullContent || !t.fullContent[language]) {
    return (
      <div style={{ color: 'red', padding: 40 }}>
        Σφάλμα: Δεν βρέθηκαν μεταφράσεις privacy policy για τη γλώσσα: {language}
      </div>
    );
  }
